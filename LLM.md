...
- `models/`: `session.py` (Session + STIX observable FKs), `fingerprint.py` (`extract_fields()` denormalizes JSONB), `heartbeat.py`, `behavioral_event.py`, `rule.py` (Rule + RuleMatch), `associations.py` (SessionURL, BrowserSession), `dashboard.py`, `user.py` (User + ApiToken), `cors.py` (AllowedOrigin), `taxii_feed.py` (TaxiiFeed), `stix.py` (9 entity models + Relationship).
- `services/`: `database.py`, `auth.py` (hash/JWT/API-token/decorators), `event_queue.py` (Redis), `mq.py` (RabbitMQ publish/consume), `schema.py` (`SCHEMA_FIELDS` registry), `intel_ingest.py` (`ingest_bundle`), `stix_store.py` (get_or_create), `stix_filters.py`, `cors_origins.py` (`dynamic_origin`), `log_shipper.py` (ships WARNING+ to Redis `ofm:logs`).
...
- `dashboards` (widgets JSONB). `users`, `api_tokens`, `allowed_origins`, `taxii_feeds`.
...
- Pages: `Dashboard/` (session table + drag-drop widgets + `FilterBuilder`, `WidgetWizard`; saved dashboards; middle-click row → new-tab `/session/:fsid`), `SessionDetail/`, `Intelligence/` (STIX browser; deep-link `/intelligence?type=&value=`; middle-click → new tab), `Logging/` (connector health + logs), `Login/`, `Landing/`, `Profile/` (password + API tokens), `Users/` (admin), `Rules/` (admin), `Exports/`.
